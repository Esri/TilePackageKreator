...
HockeyApp {
    ...
    property FileFolder fileFolder: FileFolder{ path: parentApp.folder.path + "/AppMetrics" }
    ...
    appVersion: parentApp.info.version
    ...
    function readUdid() {
        var udid = parentApp.settings.value("udid", "");
        if (!(udid > "")) {
            udid = AppFramework.createUuidString(2);
            parentApp.settings.setValue("udid", udid);
